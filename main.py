...
# https://edsacoes.streamlit.app/
# https://github.com/EdisonNunes/AnaliseAcoes
...
def carregar_dados(empresas):
    ...
    cotacoes_acao = dados_acao.history(period="1d", start="2024-01-01", end=dtfinal)
...
lista_acoes = st.sidebar.multiselect("Escolha as ações para visualizar", dados.columns, 
                                    placeholder='Escolha uma opção', default='ITUB4.SA')
...
for i, acao in enumerate(lista_acoes):
    ...
    if performance_ativo > 0:
        # :cor[texto]
        texto_performance_ativos = texto_performance_ativos + f"  \n ###### {nomes_empresas[acao]}  [ {acao} ]: :green[{performance_ativo:.1%} :+1:]"
    elif performance_ativo < 0:
        texto_performance_ativos = texto_performance_ativos + f"  \n ###### {nomes_empresas[acao]}  [ {acao} ]: :red[{performance_ativo:.1%} :-1:]"
    else:
        texto_performance_ativos = texto_performance_ativos + f"  \n ###### {nomes_empresas[acao]}  [ {acao} ]: {performance_ativo:.1%}"

total_final_carteira = sum(carteira)
performance_carteira = total_final_carteira / total_inicial_carteira - 1

if performance_carteira > 0:
    texto_performance_carteira = f"#### Performance da carteira com todos os ativos: :green[{performance_carteira:.1%} :+1:]"
elif performance_carteira < 0:
    texto_performance_carteira = f"#### Performance da carteira com todos os ativos: :red[{performance_carteira:.1%} :-1:]"
else:
